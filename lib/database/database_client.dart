...
class DatabaseClient {
  ...
  Future<bool> insertSongs() async {
    ...
    if (count != 0) {
      List<Map> results2 = await _db.rawQuery("SELECT * from songs");
  ...
  Future<int> insertOrUpdateSong(Song song) async {
    ...
    if (count == 0) {
      return await _db.insert("songs", toMap(song));
    }
    return await _db
  ...
  Future<bool> checkIfSeen() async {
    ...
    var count =
    Sqflite.firstIntValue(await _db.rawQuery("SELECT seen FROM userPrefs"));
    if (count > 0) return true;
    return false;
  ...
  Future<List<Song>> fetchSongs() async {
    List<Map> results =
        await _db.rawQuery("select * from songs order by title");
